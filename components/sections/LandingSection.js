...
export default function LandingSection() {
  return (
    <BaseSection id="landing" className="section-pad bg-white container-sm">
      {/* CTA + SOCIAL */}
          ...
          <div className="flex flex-wrap justify-center gap-3">
            {heroData.navigation.links.map((link, index) => (
              <a
                key={index}
                href={link.href}
                className="inline-block bg-gray-100 text-gray-700 px-4 py-2 rounded-lg hover:bg-neonBlue hover:text-white transition-colors duration-300 text-sm font-normal"
              >
                {link.text}
              </a>
            ))}
